      ...
      <aside className={clsx(
        ...
        <div className="flex items-center justify-between px-6 py-5 border-b border-white/10">
          <div className="flex items-center gap-3">
            {/* Logo placeholder – drop a transparent ASU SVG into public/asu-logo.svg */}
            <img
              src="/asu-logo.svg"
              alt="ASU Logo"
              className="w-10 h-10 rounded-xl object-contain"
              onError={(e) => {
                // Fallback to icon if no SVG is present
                (e.target as HTMLImageElement).style.display = 'none';
                (e.target as HTMLImageElement).nextElementSibling?.classList.remove('hidden');
              }}
            />
            <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-asu-maroon to-asu-gold flex items-center justify-center shadow-lg shadow-asu-maroon/30 hidden">
        ...
        <div className="px-6 py-4 border-b border-white/10">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-full bg-gradient-to-br from-asu-gold to-asu-maroon flex items-center justify-center text-white font-bold text-sm">
          ...
          <div className="mt-3">
            <div className="flex items-center justify-between text-xs mb-1.5">
              ...
              <span className="text-asu-gold font-medium">{student.overallProgress}%</span>
            </div>
            <div className="h-1.5 bg-white/10 rounded-full overflow-hidden">
              <motion.div initial={{ width: 0 }} animate={{ width: `${student.overallProgress}%` }}
                transition={{ duration: 1, ease: 'easeOut' }}
                className="h-full rounded-full bg-gradient-to-r from-asu-maroon to-asu-gold" />
            </div>
          </div>
        </div>
          ...
          {navItems.map((item) => (
            <NavLink key={item.to} to={item.to} end={item.to === '/'} onClick={() => setSidebarOpen(false)}
              className={({ isActive }) => clsx(
                ...
                (isActive || (item.to === '/project' && location.pathname.startsWith('/project') && !location.pathname.startsWith('/projects')))
              ...
              {({ isActive }) => (
                <>
                  <item.icon className={clsx('w-5 h-5 transition-colors',
                    (isActive || (item.to === '/project' && location.pathname.startsWith('/project') && !location.pathname.startsWith('/projects')))
                      ? 'text-asu-gold' : 'text-slate-500 group-hover:text-slate-300')} />
                  <span className="flex-1">{item.label}</span>
                  {item.to === '/project' && student.projects.length > 0 && (
                    <span className="text-[10px] font-bold bg-asu-maroon/30 text-asu-gold px-1.5 py-0.5 rounded-md">
                      {student.projects.length}
                    </span>
                  )}
                  {(isActive || (item.to === '/project' && location.pathname.startsWith('/project') && !location.pathname.startsWith('/projects'))) && (
                    <ChevronRight className="w-4 h-4 text-asu-gold" />
                  )}
                </>
              )}
            </NavLink>
          ...
          <NavLink
            ...
            className={({ isActive }) =>
              clsx(
                ...
                isActive
                  ? 'border-asu-gold/70 bg-asu-maroon/20 text-asu-gold'
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <header className="sticky top-0 z-30 bg-white/80 backdrop-blur-md border-b border-slate-200/60">
          <div className="flex items-center justify-between px-4 sm:px-6 h-16">
            ...
            <div className="hidden sm:flex items-center gap-2 px-3 py-1.5 rounded-full bg-asu-maroon/10 text-asu-maroon text-xs font-medium">
              <div className="w-1.5 h-1.5 rounded-full bg-asu-maroon animate-pulse" />
